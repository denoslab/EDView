...
**Requirements:** Must have the EDsim running locally on the same system as well.

**Starting:** 
1. First start EDsim.
2. Click the 'Open Live Map' then the map should update to show the current state of the map.
3. Go back to EDsim and start running the simulation.
...
│   ├── dashboard/
│   |   └── types.ts          # ZoneOccupancy, PatientStates, Queues...
│   ├── liveMap/
│   |   ├── expandLiveFrame.ts          # Converts input collected from simulation for usePersonaPosition.ts
│   │   ├── loadInitialState.ts         # Gets initial state of simulator and sets it up
│   │   ├── pollingSteps.ts             # Polls endpoints set up by EDsim to get updated sim state
│   │   ├── types.ts                    # MovementFile, MetaMovementFile...
|   ├── Replay/
|   │   ├── expandFrames.ts
...
4. **Furniture** — Kenney `.fbx` models loaded via `useFBX`, placed
...
| bed.fbx | bed |
| chair.fbx | chair |
| waiting_chair.fbx | waiting_room_chair |
| computer.fbx | computer |
| diagnostic_table.fbx | diagnostic_table |
| medical_cart.fbx | medical_equipment |
| wheelchair.fbx | wheelchair |
| reception_desk.fbx | reception_desk | 
